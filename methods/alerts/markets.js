...
module.exports = async () => {
  ...
  if (trendings.length > 0) {
    ...
    if (response) trendings = toArray(response).map((d, i) => ({ ...trendings.find(_d => _d.id === d.id), image: d.thumb, rank: i + 1 }));
  }
  ...
  if (minute === 0) {
    if (hour % 4 === 0) {
      ...
      if (aths.length > 0) {
        ...
        twitter_message = `${twitter_message}\n\n${data.map(d => `#${split(d.name, { delimiter: ' ' }).join('')}`).join(' ')} #Cryptocurrency`;
      ...
      if (i < 1 && telegram_messages.length < 1) {
        ...
        if (data.length > 0) {
          ...
          twitter_message = `${twitter_message}${data.length === 1 ? `\n${WEBSITE}/token/${_.head(data).id}` : `\n${WEBSITE}/tokens`}`;
          twitter_message = `${twitter_message}\n\n💙 if you HODL any one of them\n\n${data.map(d => `#${split(d.name, { delimiter: ' ' }).join('')}`).join(' ')}`;
          twitter_messages.push(twitter_message);
          telegram_messages.push(telegram_message);
        }
      }
      ...
      if (i < 5 && telegram_messages.length < 1) {
        ...
        if (data.length > 0) {
          ...
          twitter_message = `${twitter_message}${data.length === 1 ? `\n${WEBSITE}/token/${_.head(data).id}` : `\n${WEBSITE}/tokens`}`;
          twitter_message = `${twitter_message}\n\n#DeFi ${data.map(d => `#${split(d.name, { delimiter: ' ' }).join('')}`).join(' ')}`;
          twitter_messages.push(twitter_message);
          telegram_messages.push(telegram_message);
        }
      }
      if (i < 6 && telegram_messages.length < 1) {
        ...
        if (data.length > 0) {
          ...
          twitter_message = `${twitter_message}${data.length === 1 ? `\n${WEBSITE}/token/${_.head(data).id}` : `\n${WEBSITE}/tokens`}`;
          twitter_message = `${twitter_message}\n\n#NFTs ${data.map(d => `#${split(d.name, { delimiter: ' ' }).join('')}`).join(' ')}`;
          twitter_messages.push(twitter_message);
          telegram_messages.push(telegram_message);
        }
      }
