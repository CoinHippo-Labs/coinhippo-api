...
const getValueFromList = (data, fields = [], strategy = 'first') => {
  data = toArray(toArray(data).map(v => {
    let _v = v;
...
module.exports = async () => {
  ...
  data = _.orderBy(Object.entries(_.groupBy(data, 'hash')).map(([k, v]) => {
  ...
  }), ['amount_usd', 'timestamp'], ['desc', 'asc']).filter(d => d.v?.findIndex(v => v.symbol) > -1).map(d => {
    const { v } = { ...d };
    ...
    return {
      ...d,
      from_address_name: toTitle(from_address_name ? split(from_address_name, { delimiter: ' ' }).map(s => s.replace(symbol, symbol.toUpperCase())).join(' ') : symbol === 'husd' ? `${symbol.toUpperCase()} incinerator` : 'unknown wallet'),
      to_address_name: toTitle(to_address_name ? split(from_address_name, { delimiter: ' ' }).map(s => s.replace(symbol, symbol.toUpperCase())).join(' ') : 'unknown wallet'),
    };
  }).filter(d => {
  ...
  if (data.length > 0) {
    ...
    _.orderBy(_.slice(data, 0, 5), ['timestamp'], ['asc']).forEach((d, i) => {
      ...
      telegram_message = `${telegram_message}<a href="${url}">${repeatEmoji(d)} ${transaction_type ? toTitle(is_donation ? 'donation' : is_hacked ? 'stolen funds' : transaction_type) : 'transaction'}</a> <b>${numberFormat(amount, '0,0')} ${symbol.toUpperCase()}</b> <pre>$${numberFormat(amount_usd, '0,0')}</pre>\n${transaction_type === 'mint' ? `at ${to_address_name}` : transaction_type === 'burn' ? `at ${from_address_name}` : transaction_type === 'lock' ? `at ${to_address_name}` : transaction_type === 'unlock' ? `at ${to_address_name}` : `${from_address_name.replace('Unknown ', '❔')} ➡️ ${to_address_name.replace('Unknown ', '❔')}`}`;
    });
    ...
    data.forEach((d, i) => {
      ...
      twitter_message = `${twitter_message}${i > 0 ? '\n' : ''}- ${repeatEmoji(d)} ${transaction_type ? toTitle(is_donation ? 'donation' : is_hacked ? 'stolen funds' : transaction_type) : 'transaction'} ${numberFormat(amount, '0,0')} ${symbol.toUpperCase()} ($${numberFormat(amount_usd, '0,0')})\n${transaction_type === 'mint' ? `at ${to_address_name}` : transaction_type === 'burn' ? `at ${from_address_name}` : transaction_type === 'lock' ? `at ${to_address_name}` : transaction_type === 'unlock' ? `at ${to_address_name}` : `${from_address_name.replace('Unknown ', '❔')} ➡️ ${to_address_name.replace('Unknown ', '❔')}`}`;
      twitter_message = `${twitter_message}${data.length < 3 ? `\n${url}` : ''}`;
    });
    if (twitter_message) {
      twitter_message = `${twitter_message}${data.length > 2 ? '' : `\n\n${_.uniq(toArray(_.concat(data.map(d => `${d.blockchain ? `#${toTitle(d.blockchain)}` : ''}`), data.flatMap(d => [[' ', 'unknown'].findIndex(s => d.from_address_name.toLoweCase().includes(s)) < 0 && `#${toTitle(d.from_address_name)}`, [' ', 'unknown'].findIndex(s => d.to_address_name.toLoweCase().includes(s)) < 0 && `#${toTitle(d.to_address_name)}`])))).join(' ')} #WhaleAlert`}`;
    }
    if (telegram_message) {
