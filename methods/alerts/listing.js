    ...
    switch (id) {
      case 'binance':
        try {
          ...
          const response = await api.get('/public/cms/article/all/query', { params: { type: 1, pageNo: 1, pageSize: 10, sortBy: 2, apiVersion: 'V2', queryKeywords: 'list' } }).catch(error => parseError(error));
      ...
      case 'okx':
        try {
          ...
          const response = await api.get('/support/home/web').catch(error => parseError(error));
      ...
      case 'coinlist':
        try {
          ...
          const response = await website.get(path || '', { params, headers }).catch(error => parseError(error));
          const { data } = { ...response };
